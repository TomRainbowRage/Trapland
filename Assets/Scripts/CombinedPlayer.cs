...
public class CombinedPlayer : MonoBehaviour
{
    ...
    [SerializeField] public float runspeed = 40;
    ...
    [SerializeField] private Animator animator;                                 // The Animator Component used for animating.	
    ...
    float horizontalMove = 0f;          // Input Axis Raw of Horiz movement
    bool jump = false;                  // Player Jumping?
	bool jumpdown = false;
	bool jumpup = false;
    bool crouch = false;                // Player Crouching?
	...
	*/
	...
	private void Update()
    {
        ...
        if(Input.GetKeyUp(Jump))
        {
            ...
            {
                m_Rigidbody2D.velocity = new Vector2(m_Rigidbody2D.velocity.x, m_Rigidbody2D.velocity.y * cutofMulti);
            }
        }
        */
	...
	private void FixedUpdate()
	{
		...
		jumpdown = Input.GetKeyDown(AltJump);
		jump = Input.GetKey(AltJump);
		jumpup = Input.GetKeyUp(AltJump);

        Move(horizontalMove * Time.fixedDeltaTime, crouch, jump);
        //jump = false;
		if(animator.enabled == false && m_Grounded)
        {
	...
	public void Move(float move, bool crouch, bool jump)
	{
		...
		*/
		if(m_Grounded && jumpdown)
		{
			...
			isJumping = true;
            jumpTime = jumpStartTime;
            m_Rigidbody2D.velocity = Vector2.up * (m_JumpForce / 100);
		}
